                ...
                <div class="row" style="margin-top: 50px">
                    <div class="col-4 ">
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="text" class="form-control search_friends" name="search"  placeholder="friend name" aria-label="Username" aria-describedby="basic-addon1">
                        </div>
                        ...
                        <div class="list-group nav-pills " data-status="1" id="list-tab" role="tablist">
                            @if ($auth_friends->count() > 0)
                                @foreach ($auth_friends as $i => $user)
                                    <button
                                        class="friends_1_page user_btn nav-link list-group-item list-group-item-action {{ $i == 0 ? 'active index_0' : null }}"
                                    ...
                                    <div class="tab-pane fade  {{ $i == 0 ? 'show active' : null }}"
                                        ...
                                        <form id={{ 'form' . $user->id }}>
                                            <div class="card" style="height: 300px">
                                                ...
                                                <div class="card-body box{{ $user->id }}" 
                        ...
                        <div class="list-group nav-pills " data-status="1" id="list-tab" role="tablist">
                            @if ($friends_msgs->count() > 0)
                                @foreach ($friends_msgs as $i => $msg)
                                    <button
                                        class="user_btn nav-link list-group-item list-group-item-action "
                                        id="list-home-list" data-bs-toggle="pill"
                                        data-bs-target={{ '#msg_box' . $msg->sender->id !='#msg_box'. Auth::id()?'#msg_box'. $msg->sender->id :'#msg_box'.$msg->receiver->id}} role="tab"
                                        data-id="{{ $msg->sender->id != Auth::id()? $msg->sender->id :$msg->receiver->id }}" aria-controls="home"
                                        data-index="{{ $i }}" data-status="0" style="height:95px">
                            ...
                            @if ($friends_msgs->count() > 0)
                                @foreach ($friends_msgs as $i => $msg)
                                    <div class="tab-pane fade  "
                                        id={{ 'msg_box' . $msg->sender->id !='msg_box'. Auth::id()?'msg_box'. $msg->sender->id :'msg_box'.$msg->receiver->id }} role="tabpanel" aria-labelledby="list-home-list">
                                        <form id={{ 'form' . $msg->sender->id !='form'. Auth::id() ? 'form'. $msg->sender->id :'form'.$msg->receiver->id }}>
                                            ...
                                            <div class="card" style="height: 300px">
                                                ...
                                                <input type="hidden" name="receiver_id" class="receiver_id"
                                                    value="{{ $user->id }}">
                                                <input type="text" name="message" id="{{ 'msg' . $msg->sender->id !='msg'. Auth::id() ? 'msg'. $msg->sender->id :'msg'.$msg->receiver->id }}"
                                                    class="form-control send_input" data-id="{{  $msg->sender->id != Auth::id() ?  $msg->sender->id : $msg->receiver->id }}">
